...
def generate_html_from_text_lines(lines, filename=None):
    filtered_lines = [line for line in lines if line.strip()]  # usuń puste linie
    content_blocks = "\n".join(
    ...
    <body>
        <h3>Dokument XML</h3>
        <h4 class="acc">Zawartość pliku:</h4>
        {filename_display}
        {content_blocks}
    </body>
    </html>
...
class XMLViewerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Uniwersalny podgląd dokumentu XML - wersja 1.0 autor EZ")
        self.root.geometry("1500x900")

        self.open_button = tk.Button(root, text="Wczytaj plik XML", command=self.load_xml)
        self.open_button.pack(pady=10)

        self.print_button = tk.Button(root, text="Podgląd do wydruku", command=self.print_html)
        self.print_button.pack(pady=5)

        self.attachments_info_label = tk.Label(root, text="", font=("Arial", 12), justify="left", anchor="w")
        self.attachments_info_label.pack(fill="x", padx=20, pady=(5, 10))
        ...
        self.html_view = HTMLLabel(frame, html="<p>Tu pojawi się podgląd dokumentu</p>", background="white")
        self.html_view.pack(side="left", fill="both", expand=True)
    ...
    def load_xml(self):
        ...
        try:
            ...
            if attachments:
                ...
                self.attachments_info_label.config(text="")
    ...
    def show_attachments_info(self):
        ...
        full_text = "\n".join(info_lines)
        self.attachments_info_label.config(
            text=full_text,
            font=("Arial", 12, "bold"),
            justify="left",
            anchor="w"
